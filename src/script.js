...
document.addEventListener(
	...
	(e) => {
		...
		/** this give exactly position of the cursor over the element */
		// this should run in the raf bc when stop moving the cursor doesnt update
		mouseX = e.clientX //- Math.floor(window.innerWidth);
		mouseY = e.clientY //- Math.floor(window.innerHeight);
		// with lerp
		// currX = currX + (mouseX - currX) * smooth;
		// currY = currY + (mouseY - currY) * smooth;  
		// customCursor.style.cssText = `left: ${currX}px; top: ${currY}px;`;
    const x = e.clientX;
		const y = e.clientY;
		// customCursor.style.cssText = `left: ${cursor.x}px; top: ${cursor.y}px;`;// doesnt work
		// customCursor.style.cssText = `left: ${x}px; top: ${y}px;`;// doesnt work
	},
	false
);
...
//////////////////////////////////////////////////
//// MAGNETIC MENU
const menuNav = document.querySelectorAll("nav > .a");
// const customCursor = document.querySelector(".cursor");
...
menuNav.forEach((b) => b.addEventListener("mousemove", update));
menuNav.forEach((b) => b.addEventListener("mouseleave", update));
